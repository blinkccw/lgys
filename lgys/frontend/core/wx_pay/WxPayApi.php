...
class WxPayApi {
     ...
     * @return 成功时返回，其他抛异常
     */
    public static function unifiedOrder($config, $inputObj, $notify_url, $timeOut = 6) {
...
//        }
        ...
        $inputObj->SetSub_appid($config['SUBAPPID']); //公众账号ID
        $inputObj->SetSub_mch_id($config['SUBMCHID']); //商户号
        $inputObj->SetSpbill_create_ip($_SERVER['REMOTE_ADDR']); //终端ip	  
        //$inputObj->SetSpbill_create_ip("1.1.1.1");  	    
        $inputObj->SetNonce_str(self::getNonceStr()); //随机字符串
        //签名
        $inputObj->SetSign($config['KEY']);
        $xml = $inputObj->ToXml();
        Yii::info($xml);
        $startTimeStamp = self::getMillisecond(); //请求开始时间
        $response = self::postXmlCurl($config, $xml, $url, false, $timeOut);
        $result = WxPayResults::Init($response, $config['KEY']);
        Yii::info($result);
    ...
    public static function orderQuery($config, $inputObj, $timeOut = 6) {
        ...
        $inputObj->SetSign($config['KEY']); //签名
        $xml = $inputObj->ToXml();
