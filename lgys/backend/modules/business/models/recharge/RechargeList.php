<?php

namespace backend\modules\business\models\recharge;

use Yii;
use common\models\BusinessPoints;
use common\core\BasePageModel;

/**
 * 商户充值列表
 */
class RechargeList extends BasePageModel {

    public $id;
    public $begin_at;
    public $end_at;

    /**
     * @inheritdoc
     */
    public function rules() {
        $rules = [
            ['id', 'trim'],
            ['id', 'required'],
            ['id', 'integer'],
            [['begin_at', 'end_at'], 'trim'],
            [['begin_at', 'end_at'], 'safe']
        ];
        return array_merge($rules, parent::rules());
    }

    /**
     * @inheritdoc
     */
    public function attributeLabels() {
        $labels = [
            'id' => '商户ID',
            'begin_at' => '开始日期',
            'end_at' => '结束日期'
        ];
        return array_merge($labels, parent::attributeLabels());
    }

    /**
     * 列表
     */
    public function getList($page_size = 10) {
        $this->page_size = $page_size;
        $data['business_id'] = $this->id;
        $data['points_type'] = 1;
        $dataPage = BusinessPoints::find()->where($data);
        if ($this->begin_at != '') {
            $dataPage->andWhere(['>=', 'created_at', $this->begin_at]);
        }
        if ($this->end_at != '') {
            $dataPage->andWhere(['<=', 'created_at', $this->end_at . ' 23:59:59']);
        }
        $counts = $dataPage->count();
        $list = $dataPage->orderBy('id desc')
                        ->offset(($this->page_index - 1) * $this->page_size)
                        ->limit($this->page_size)
                        ->asArray()->all();
        $rel['list'] = $list;
        $rel['page'] = $this->getPageData($counts);
        return $rel;
    }

}
